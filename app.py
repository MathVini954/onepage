...
# ✅ Resumo global (tabela de todas as obras)
...
# ============================================================
# Tema (blindado: NÃO some com a tela)
# ============================================================
...
# ============================================================
with tab_resumo:
    st.subheader("ORÇAMENTO_RESUMO — Todas as obras")

    if df_orc_resumo is None or df_orc_resumo.empty:
        st.info("Sem a aba **ORÇAMENTO_RESUMO** no Excel (ou ela está vazia).")
    else:
        # organizar: OBRA primeiro, depois meses, depois VARIAÇÃO
        cols = list(df_orc_resumo.columns)
        obra_col = next((c for c in cols if c.strip().upper() == "OBRA"), cols[0])
        var_col = next((c for c in cols if "VARIA" in c.upper()), None)

        month_cols = [c for c in cols if c not in (obra_col, var_col)]
        show_cols = [obra_col] + month_cols + ([var_col] if var_col else [])

        df_show = df_orc_resumo[show_cols].copy()
        # format money
        fmt_map = {c: fmt_brl for c in df_show.columns if c != obra_col}
        st.dataframe(df_show.style.format(fmt_map), use_container_width=True, hide_index=True)

        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

        # gráfico rápido: variação por obra (se existir)
        if var_col:
            vv = pd.to_numeric(df_orc_resumo[var_col], errors="coerce")
            dfx = df_orc_resumo[[obra_col, var_col]].copy()
            dfx[var_col] = vv
            dfx = dfx.dropna()
            if not dfx.empty:
                fig = go.Figure()
                ...
                fig.update_layout(height=320, yaxis_title="R$")
                st.plotly_chart(apply_plotly_theme(fig), use_container_width=True)
